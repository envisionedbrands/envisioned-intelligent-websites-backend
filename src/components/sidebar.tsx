...
const NAV_ITEMS = [
  ...
  {
    // Yours, not upstream's — the Operator agent (src/app/agent, migration 200).
    href: '/agent',
    label: 'Operator',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 shrink-0" viewBox="0 0 256 256" fill="currentColor">
...
function ThemeToggle({ collapsed }: { collapsed: boolean }) {
  ...
  const toggle = () => {
    const next = !light;
    setLight(next);
    // Both classes, to match the pre-paint script in layout.tsx. Upstream only
    // toggles `dark`; the Envisioned Winter palette in globals.css is keyed on
    // `html.light`, so without adding that class light mode silently falls back
    // to upstream's generic white/zinc inversion.
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center px-3 mb-6 ${collapsed ? 'flex-col gap-3' : 'justify-between'}`}>
        <Link href="/content" className="flex items-center gap-2.5 px-1.5" title="Envisioned — Digital Home">
          <span className="w-6 h-6 rounded-md bg-white text-black text-[11px] font-bold flex items-center justify-center shrink-0">
            E
          </span>
          {!collapsed && <span className="text-[14px] font-semibold text-white">Envisioned</span>}
        </Link>
        <button
